...
public class GuitarTuner extends AppCompatActivity {
    ...
    private static final String TAG = "Guitar Tuner";
    ...
    private void processRecordedData() {
        ...
        recordedDataFFT = FrequencyDetectorFFT.convertToFloats(mRecordedData);
        fft.realForward(recordedDataFFT);

        float[] halfAbsFFT;
        halfAbsFFT = FrequencyDetectorFFT.absAndHalve(recordedDataFFT);

        int[] peaks;
        peaks = FrequencyDetectorFFT.identifyPeaksAndRemove(halfAbsFFT);

        String peakString = "";
        for(int e : peaks) peakString += e + " ";
        Log.d(TAG, peakString);

        int frequency = FrequencyDetectorFFT.detectFrequency(peaks);
    ...
    private void updateTuner(final int frequency) {
        ...
        final int stringFreq = FrequencyDetectorFFT.detectStringFreq(frequency);
        final String stringName = FrequencyDetectorFFT.getStringName(stringFreq);
